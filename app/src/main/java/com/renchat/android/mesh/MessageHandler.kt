...
class MessageHandler(private val myPeerID: String) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.renchat.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.renchat.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    ...
                    val privateMessage = com.renchat.android.model.PrivateMessagePacket.decode(noisePayload.data)
                    if (privateMessage != null) {
                        Log.d(TAG, "🔓 Decrypted TLV PM from $peerID: ${privateMessage.content.take(30)}...")
                        
                        // Create RenChatMessage exactly like iOS
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.renchat.android.model.NoisePayload(
                type = com.renchat.android.model.NoisePayloadType.DELIVERED,
     ...
     */
    private suspend fun handlePrivateMessage(packet: RenChatPacket, peerID: String) {
        try {
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: RenChatPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: RenChatPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: RenChatMessage)
