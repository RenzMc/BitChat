...
import com.renchat.android.onboarding.BluetoothCheckScreen
import com.renchat.android.onboarding.BluetoothStatus
import com.renchat.android.onboarding.BluetoothStatusManager
import com.renchat.android.onboarding.BatteryOptimizationManager
import com.renchat.android.onboarding.BatteryOptimizationScreen
import com.renchat.android.onboarding.BatteryOptimizationStatus
import com.renchat.android.onboarding.InitializationErrorScreen
import com.renchat.android.onboarding.InitializingScreen
import com.renchat.android.onboarding.LocationCheckScreen
import com.renchat.android.onboarding.LocationStatus
import com.renchat.android.onboarding.LocationStatusManager
import com.renchat.android.onboarding.OnboardingCoordinator
import com.renchat.android.onboarding.OnboardingState
import com.renchat.android.onboarding.PermissionExplanationScreen
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle deep links from initial intent
        handleDeepLink(intent)
    }
    
    private fun handleDeepLink(intent: Intent?) {
        intent?.let { intentObj ->
            val data = intentObj.data
            if (data != null) {
                Log.d("MainActivity", "Deep link received: $data")
                
                when (data.scheme) {
                    "renchat" -> {
                        handleRenChatDeepLink(data)
                    }
                    "https", "http" -> {
                        if (data.host == "renchat.app") {
                            handleRenChatWebLink(data)
                        }
                    }
                }
            }
    ...
    private fun handleRenChatDeepLink(data: android.net.Uri) {
        when (data.host) {
            "group" -> {
                when (data.path) {
                    "/join" -> {
                        val inviteCode = data.getQueryParameter("invite")
                        if (inviteCode != null) {
                            Log.d("MainActivity", "Group join invite: $inviteCode")
    ...
    private fun handleRenChatWebLink(data: android.net.Uri) {
        // Handle web links that should redirect to app functionality
        // This allows browser-based sharing to work properly
        when (data.path) {
            "/group/join" -> {
                val inviteCode = data.getQueryParameter("invite")
                if (inviteCode != null) {
                    Log.d("MainActivity", "Web group join invite: $inviteCode")
                    handleGroupInvite(inviteCode)
                }
            }
        }
    ...
    private fun handleGroupInvite(inviteCode: String) {
        // Ensure app is fully initialized before processing group invite
        if (mainViewModel.onboardingState.value == OnboardingState.COMPLETE) {
            // App is ready, process group invite immediately
            processGroupInvite(inviteCode)
        } else {
            // App not ready yet, queue the invite to process after onboarding
            Log.d("MainActivity", "Queuing group invite for after onboarding: $inviteCode")
            // Store invite code to process later
            pendingGroupInvite = inviteCode
        }
    }
    
    private var pendingGroupInvite: String? = null
    
    private fun processGroupInvite(inviteCode: String) {
        try {
            val success = chatViewModel.joinGroupByInvite(inviteCode)
            if (success) {
                Log.d("MainActivity", "Successfully joined group via invite: $inviteCode")
                // Optionally show a toast or navigate to the group
            } else {
                Log.w("MainActivity", "Failed to join group via invite: $inviteCode")
            }
        } catch (e: Exception) {
            Log.e("MainActivity", "Error processing group invite: ${e.message}")
        }
    }
    ...
    private fun handleOnboardingStateChange(state: OnboardingState) {
        ...
        when (state) {
            OnboardingState.COMPLETE -> {
                ...
                // Process any pending group invites
                pendingGroupInvite?.let { inviteCode ->
                    Log.d("MainActivity", "Processing pending group invite: $inviteCode")
                    processGroupInvite(inviteCode)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.renchat.android.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        if (shouldOpenPrivateChat) {
            val peerID = intent.getStringExtra(com.renchat.android.ui.NotificationManager.EXTRA_PEER_ID)
            val senderNickname = intent.getStringExtra(com.renchat.android.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
