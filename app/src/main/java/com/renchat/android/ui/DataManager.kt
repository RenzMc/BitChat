...
class DataManager(private val context: Context) {
    ...
    private val prefs: SharedPreferences = context.getSharedPreferences("RenChat_prefs", Context.MODE_PRIVATE)
    ...
    // MARK: - Geohash Channel Persistence (from BitChat 1.1.0)
    ...
    fun loadLockedChats() {
        val savedLockedChats = prefs.getStringSet("locked_chats", emptySet()) ?: emptySet()
        _lockedChats.addAll(savedLockedChats)
        Log.d(TAG, "Loaded ${savedLockedChats.size} locked chats from storage: $savedLockedChats")
    }
    
    fun saveLockedChats() {
        prefs.edit().putStringSet("locked_chats", _lockedChats).apply()
        Log.d(TAG, "Saved ${_lockedChats.size} locked chats to storage: $_lockedChats")
    }
    
    fun lockChat(peerID: String) {
        val wasAdded = _lockedChats.add(peerID)
        Log.d(TAG, "lockChat: peerID=$peerID, wasAdded=$wasAdded")
        saveLockedChats()
    }
    
    fun unlockChat(peerID: String) {
        val wasRemoved = _lockedChats.remove(peerID)
        Log.d(TAG, "unlockChat: peerID=$peerID, wasRemoved=$wasRemoved")
        saveLockedChats()
    }
    
    fun isChatLocked(peerID: String): Boolean {
        val result = _lockedChats.contains(peerID)
        Log.d(TAG, "isChatLocked check: peerID=$peerID, result=$result")
        return result
    }
