...
import com.renchat.android.mesh.PeerFingerprintManager
...
class PrivateChatManager(
    ...
    private val unreadReceivedMessages = mutableMapOf<String, MutableList<RenChatMessage>>()
    ...
    fun handleIncomingPrivateMessage(message: RenChatMessage) {
...
//    }
    ...
    private fun getPeerIDForNickname(nickname: String, meshService: BluetoothMeshService): String? {
        ...
        Log.d(TAG, "Looking for peer with nickname: '$cleanNickname' (original: '$nickname')")
        
        val peerNicknames = meshService.getPeerNicknames()
        Log.d(TAG, "Available peer nicknames: $peerNicknames")
        
        // First try exact match
        val exactMatch = peerNicknames.entries.find { it.value == cleanNickname }?.key
        if (exactMatch != null) {
            Log.d(TAG, "Found exact match for '$cleanNickname': $exactMatch")
            return exactMatch
        }
        
        // Try case-insensitive match
        val caseInsensitiveMatch = peerNicknames.entries.find { 
            it.value.equals(cleanNickname, ignoreCase = true) 
        }?.key
        if (caseInsensitiveMatch != null) {
            Log.d(TAG, "Found case-insensitive match for '$cleanNickname': $caseInsensitiveMatch")
            return caseInsensitiveMatch
        }
        
        // Try partial match (contains)
        val partialMatch = peerNicknames.entries.find { 
            it.value.contains(cleanNickname, ignoreCase = true) || 
            cleanNickname.contains(it.value, ignoreCase = true)
        }?.key
        if (partialMatch != null) {
            Log.d(TAG, "Found partial match for '$cleanNickname': $partialMatch")
            return partialMatch
        }
        
        Log.w(TAG, "No peer found with nickname '$cleanNickname'")
        return null
    }
