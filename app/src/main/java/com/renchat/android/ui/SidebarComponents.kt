        ...
        Row(
            ...
            Column(
                ...
                LazyColumn(
                    ...
                    // Groups section
                    val userGroups = viewModel.getUserGroups()
                    if (userGroups.isNotEmpty()) {
                        item {
                            GroupsSection(
                                ...
                                onGroupClick = { channel ->
                                    viewModel.switchToChannel(channel)
                                    onDismiss()
                                },
                                onLeaveGroup = { channel ->
                                    viewModel.leaveChannel(channel)
                                },
                                unreadGroupMessages = unreadChannelMessages // Groups use same unread system
                            )
                        }
                    ...
                    // Regular channels section (filter out group channels)
                    val regularChannels = joinedChannels.filter { !it.startsWith("#group:") }
                    if (regularChannels.isNotEmpty()) {
                        item {
                            ChannelsSection(
                                channels = regularChannels, // Only show regular channels
                                currentChannel = currentChannel,
                                colorScheme = colorScheme,
