...
class MeshDelegateHandler(
    ...
    // Anti-spam protection for incoming messages
    private var antiSpamManager: AntiSpamManager? = null
    
    fun initializeAntiSpam(context: android.content.Context) {
        antiSpamManager = AntiSpamManager.getInstance(context)
    }
    ...
    override fun didReceiveMessage(message: BitchatMessage) {
        coroutineScope.launch {
            ...
            message.senderPeerID?.let { senderPeerID ->
                ...
                antiSpamManager?.let { manager ->
                    if (manager.isUserMuted(senderPeerID)) {
                        android.util.Log.d("MeshDelegateHandler", "Blocked message from muted user: $senderPeerID")
                        return@launch
                    }
                }
            }
